...
for ip = 1:numel(t.oplist)
    ...
    %disp(input{2}(:,1));
    %disp(HW(bitxor(state, input{2}(:,1))))
    ...
    for iv = 1:size(modelList,1)
        ...
        if ((strcmpi(modelList{iv,2},'I1+I3+D+D*I1+D*I3+o1int+o2int') && ismember(ip,[2])) || ...
                (strcmpi(modelList{iv,2},'I1+I3+D+D*I1+D*I3+o1int+o2int+ow') && ismember(ip,[5])) || ...
                (strcmpi(modelList{iv,2},'I1+I3+D+D*I1+D*I3+ow') && ismember(ip,[1])) )
