...
clear;
t = load('m-ldr.mat');
ss = load('state-ldr.mat');
%t = load('m_ldr+lsls_5(1).mat');
%ss = load('state_ldr+lsls_5(1).mat');
%
...
Xcols = describeX(Xcols,'s','HD Mem bus state & operand stored/loaded',[1],NaN);
Xcols = describeX(Xcols,'ow','HD overwrite',[1],NaN);
%Xcols = describeX(Xcols,'IS','State instruction (LSL/STR/LDR/MUL)',[1:nIS-1],NaN);
...
%     'I1+I3+D+D*I1+D*I3+o1adj+o2adj');
 ...
 modelList = addModel(modelList,Xcols,{'Const','I1','I3','D','D*I1','D*I3','o1int','o2int','ow'},[],...
     'I1+I3+D+D*I1+D*I3+o1int+o2int+ow'); 
...
%testPairs = compareModel(testPairs,modelList,'I1','I1+lsls','lsls');
...
%testPairs = compareModel(testPairs,modelList,'I1+I3+D+D*I1+D*I3+s','I1+I3+D+D*I1+D*I3+s+ow','OW(*)');
%testPairs = compareModel(testPairs,modelList,'I1+I3+D+D*I1+D*I3+ow','I1+I3+D+D*I1+D*I3+s+ow','OW(*)');
%testPairs = compareModel(testPairs,modelList,'I1+I3+D+D*I1+D*I3','I1+I3+D+D*I1+D*I3+o1int+o2int','OW(*)');
%testPairs = compareModel(testPairs,modelList,'I1+I3+D+D*I1+D*I3','I1+I3+D+D*I1+D*I3+lsls','LSLS(*)');
...
testPairs = compareModel(testPairs,modelList,'I1+I3+D+D*I1+D*I3+o1int+o2int','I1+I3+D+D*I1+D*I3+o1int+o2int+ow','Non-adjacent bits'); 
...
for ip = 1:numel(t.oplist)
    ...
    dummyIS=zeros(numel(opcodes(:,3)),4);
    dummyIS(:,1)=1;
    ...
    %end
    A(:,Xcols{strcmpi(Xcols(:,1),'s'),3}) =  HW(bitxor(state, input{2}(:,2)));
    A(:,Xcols{strcmpi(Xcols(:,1),'ow'),3}) = HW(bitxor(input{2}(:,1),input{2}(:,2)));
    
    %A(:,Xcols{strcmpi(Xcols(:,1),'IS'),3}) = dummyIS(:,2:end);
    ...
    for iv = 1:size(modelList,1)
        ...
        if ((strcmpi(modelList{iv,2},'I1+I3+D+D*I1+D*I3+o1int+o2int+ow') && ismember(ip,[2,5])) || ...
                (strcmpi(modelList{iv,2},'I1+I3+D+D*I1+D*I3+s+ow') && ~ismember(ip,[2,5])))
